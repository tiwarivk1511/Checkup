...
export default function SwipeValueBasedUI() {
  ...
  // Fetching the Appointments from API
  async function fetchAppointments() {
    const url = 'http://192.168.43.180:8000';
    try {
      ...
      // date="2023-06-23";
      const response = await fetch(url + '/api/appointments?date=2023-07-26');
      ...
      if (response.ok) {
        ...
        setFilteredData(sortedAppointments);

        if (sortedAppointments.length !== 0) {
          if (sessionStorage.getItem('first_loaded') === 'true') {
            sessionStorage.setItem('defaultPatient', sortedAppointments[0].Patient.id);
  ...
  // Searching Operation performance
  ...
  async function fetchData() {
    const url = 'http://192.168.43.180:8000/api/patients';
    try {
  ...
  // Handle Press when clicking on Search
  ...
  // Handle Press when clicking on Add New
  ...
  async function cancelAppointments(item) {
    const URL = 'http://192.168.43.180:8000/api/appointment/cancel?id=' + item.id;
  ...
  return (
    <TouchableOpacity style={styles.container} onPress={handleScreenTouch}>
      ...
      {/* Add the Appointments Card here */}
      <View>
        <FlatList data={appointmentlist} renderItem={DataCards} keyExtractor={(item) => item.id.toString()} />
      </View>

      {/* Floating Action Button */}
      <View style={styles.btnView}>
        {/* Add the cards here */}
        <FlatList data={filteredData} renderItem={DataCards} keyExtractor={(item) => item.id.toString()} />
      </View>
      ...
      <Modal visible={showPopup} animationType="slide" transparent={true}>
        <View style={styles.popup}>
          ...
          <TouchableOpacity style={styles.popupButton} onPress={handlePress}>
            <Text style={styles.popupButtonText}>Search</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.popupButton} onPress={handlePressOnAddNew}>
        ...
        <TouchableOpacity onPress={handleScreenTouch}>
          <View style={styles.backView}>
            <View style={styles.frontView}>
              {searchQuery && (
                <TouchableOpacity style={styles.clrBtn} onPress={handleClear}>
                  <Image source={require('./images/clear.png')} />
                </TouchableOpacity>
              )}

              {/* Getting input from the user for the search */}
              <TextInput
                style={styles.input}
                placeholder="Search"
                value={searchQuery}
              ...
              <ScrollView horizontal={true}>
                <FlatList data={filteredData} renderItem={SearchCards} keyExtractor={(item) => item.id.toString()} />
              </ScrollView>
            </View>
          </View>
        </TouchableOpacity>
          ...
          <View style={styles.PbackView}>
            <View style={styles.PfrontView}>
              <View style={styles.fontView}>
                <Text style={styles.font}>Patient's Details</Text>
              </View>

              <TextInput style={styles.PInput} placeholder="Name" />
              <TextInput style={styles.PInput} placeholder="Age" />
              <TextInput style={styles.PInput} placeholder="Gender" />
              <TextInput style={styles.PInput} placeholder="D.O.B." />
              <TextInput style={styles.PInput} placeholder="Ph. Number" />
              <TextInput style={styles.PInput} placeholder="Address" />
              <TextInput style={styles.PInput} placeholder="E-mail" />
              <TextInput style={styles.uniqueID} placeholder="Unique ID" />

              <TouchableOpacity style={styles.btnGen}>
                <Text style={styles.genTxt}>Generate</Text>
              </TouchableOpacity>
